...
function draw_force_main(transition){
    ...
    for(let j in machine_data.ms2_metrics){
        ...
        if(machine_data.ms2_metrics[j].display_name == "Precursor Mass Error"){
            ...
            newScale.domain([machine_data.ms2_metrics[j].stats.max, 0])
        }
        edgeScales[machine_data.ms2_metrics[j].metric_id] = newScale;
    }
    ...
    .attr("filter", function(d){
        if(d.nodeType == 1 || d.nodeType == 3){return "url(#specular)"}
        //if(d.nodeType == 2 || d.nodeType == 4){return "url(#diffuse)"}
        //return "url(#specular)";
    })
    ...
    .on("mouseover", function(event, d){
        ...
        if(d.nodeType == 1){
            ...
            if(miss.includes(run_index)){
                ...
                if(d.name == "Precursor Mass Error"){
                    var newText = d.value.toFixed(2);
                }
                else{
                    var newText = parseInt(d.value); 
                }
            }
        }
...
function getGraph(w, h){
    ...
    var node_positions = {"Mass Error (mDa)": [0.1*w, 0.25*h],
                        "Mass Error (ppm)": [0.3*w, 0.25*h],
                        "Retention Time": [0.5*w, 0.25*h],
                        "Tailing": [0.7*w, 0.25*h],
                        "Asymmetry": [0.9*w, 0.25*h],
                        "MS2 Metrics": [0.2*w, 0.75*h],
                        "Full Width Half Maximum": [0.4*w, 0.75*h],
                        "Area (normalised)": [0.6*w, 0.75*h],
                        "Height (normalised)": [0.8*w, 0.75*h]};
    ...
    // MS2 metric node
    var new_node = {"id": "MS2 Metrics",
                        "name": "MS2 Metrics",
                        "metricName": "MS2 Metrics",
                        "radius": 4*currentRadius,  
                        "nodeType": 4,
                        "fx": node_positions["MS2 Metrics"][0], 
                        "fy": node_positions["MS2 Metrics"][1],
                        "x": node_positions["MS2 Metrics"][0], 
                        "y": node_positions["MS2 Metrics"][1],
                        "colour": "var(--dark)"};

    nodes.push(new_node);

    for(let j in machine_data.ms2_metrics){
        var new_node = {"id": machine_data.ms2_metrics[j].display_name,
                        "name": machine_data.ms2_metrics[j].display_name,
                        "metricName": machine_data.ms2_metrics[j].display_name,
                        "radius": currentRadius,  
                        "nodeType": 1,
                        "x": node_positions["MS2 Metrics"][0], 
                        "y": node_positions["MS2 Metrics"][1],
                        "mIndex": j,
                        "value": machine_data.ms2_metrics[j].values[run_index],
                        "colour": machine_data.ms2_metrics[j].colour};

        var new_link = {"source": "MS2 Metrics", 
                        "target": machine_data.ms2_metrics[j].display_name,
                        "targetName": machine_data.ms2_metrics[j].display_name,  
                        "linkType": 2,
                        "value": machine_data.ms2_metrics[j].values[run_index],
                        "linkScale": machine_data.ms2_metrics[j]["metric_id"],
                        "colour": machine_data.ms2_metrics[j]["colour"]};
    ...
    // nodeType 1: ms2 components
    // nodeType 2: ms2 base node
    // nodeType 3: lcms components
    // nodeType 3: lcms base nodes
    graph["base_nodes"] = graph.nodes.filter(d => d.nodeType == 2 || d.nodeType == 4);
    return graph;
}
...
function dragstarted(event, d) {
  ...
  function updateRadiosLineChart(displayName){
    ...
    var ms2_metrics = ["MS/MS Spectra", "Target PSMs", "Unique Target Peptides",
    ...
    else if(ms2_metrics.includes(display_metric)){
        document.getElementById("box").disabled = true;
        document.getElementById("box").style.boxShadow = "";
        document.getElementById("box").classList = "btn";
        document.getElementById("heat").disabled = true;
